...
{
	struct rte_mbuf *first_seg, *prev_seg, *cur_seg;
	uint8_t i = 0;
			   ...
			   struct armada_ep_rxq *sw_rxq, uint16_t nb_descs)
{
	...
	if (additional_descs > 0) {
		 ...
		 */
		new_desc_idx = num_sg_entries - additional_descs - 1;
...
armada_ep_rx_pkt_burst(void *rxq, struct rte_mbuf **rx_pkts, uint16_t nb_descs)
{
	...
	if (rte_spinlock_trylock(&priv->bp_lock) == 1) {
		...
		} else if (unlikely(num > priv->bp_max_size))
			ARMADA_EP_LOG(ERR, "bpool is overloaded (%d vs %d)!\n",
				      num, priv->bp_max_size);
...
/*
 * armada_ep_put_qeueu_idx - Free a previous allocated entry for queue
 * control pointers.
 */
static inline void
armada_ep_put_qeueu_idx(struct armada_ep_priv *priv, uint32_t index)
{
...
armada_ep_free_queue_resources(struct armada_ep_queue *hw_queue)
{
	...
	if (hw_queue->desc) {
		rte_free(hw_queue->desc);
		hw_queue->desc = NULL;
	};

	armada_ep_put_qeueu_idx(hw_queue->priv, hw_queue->prod_idx);
	armada_ep_put_qeueu_idx(hw_queue->priv, hw_queue->cons_idx);
	return 0;
}
 ...
 * The fune rte_eth_dev_release_port() will free the following:
...
armada_ep_dev_close(struct rte_eth_dev *dev)
{
	...
	armada_ep_free_queue_resources(&priv->notif_queue);
	armada_ep_free_queue_resources(&priv->cmd_queue);
#endif

	for (i = 0; i < priv->num_rx_queues; i++) {
		armada_ep_free_queue_resources(priv->rx_queue[i]);
		armada_ep_free_queue_resources(&priv->bp_queue[i]);
	...
	for (i = 0; i < priv->num_tx_queues; i++) {
		...
		armada_ep_free_queue_resources(priv->tx_queue[i]);
		for (j = 0; j < RTE_MAX_LCORE; j++) {
 ...
 * @param alloc_b_info
 *   Define if cookie list allocation is required. ALLOC_COOKIES if allocation
 *   is required, alse DONT_ALLOC_COOKIES.
 *
 * @return
 *   0 on success, negative error value otherwise.
 */
static inline int
armada_ep_alloc_queue_resources(struct armada_ep_queue *queue, int desc_size,
				int queue_len, int alloc_b_info)
{
	int ret = -ENOMEM;
	...
	queue->desc = rte_zmalloc("desc", queue_len * desc_size, 0);

	if (!queue->desc)
		return -ENOMEM;

	queue->dma = rte_mem_virt2phy(queue->desc);
	...
	if (ret < 0) {
		ARMADA_EP_LOG(ERR, "Unable to allocate entry for queue control"
			      " ptrs.\n");
		goto err;
	}
	...
	ret = armada_ep_get_queue_idx(queue->priv);
	if (ret < 0) {
		ARMADA_EP_LOG(ERR, "Unable to allocate entry for ring control "
			      "pointers.\n");
		goto err;
	}
...
err:
	...
	queue->cons_p = NULL;
	if (queue->cons_idx)
		armada_ep_put_qeueu_idx(queue->priv, queue->cons_idx);
	if (queue->prod_idx)
		armada_ep_put_qeueu_idx(queue->priv, queue->prod_idx);
	if (queue->desc)
		rte_free(queue->desc);
	queue->desc = NULL;
...
/* init rte_eth_dev struct with callback functions*/
	 ...
	 * maximum number of descriptors in max of rx queues size plus minimum
	 * bpool size.
	 * In case the bpool size will exceed this value, superfluous buffers
	 * will be removed
	 */
	priv->bp_max_size = ARMADA_EP_RXD_MAX + priv->bp_min_size;
 ...
 */
static int
armada_ep_dev_infos_get(struct rte_eth_dev *dev __rte_unused,
			struct rte_eth_dev_info *dev_info)
{
	...
	/* TODO: S/G currently set by default, need to read from hw cap */
	...
	dev_info->max_rx_pktlen = ARMADA_EP_PKT_SIZE_MAX;
	return 0;
}
			 ...
			 struct rte_mempool *mp)
{
	...
	min_size = buff_size - RTE_PKTMBUF_HEADROOM - ARMADA_EP_PKT_EFFEC_OFFS;
	...
	ret = armada_ep_alloc_queue_resources(hw_rxq, desc_size,
					      priv->rx_queue_size, 0);
	if (ret) {
		ARMADA_EP_LOG(ERR, "Failed to allocate rx ring %d.\n", idx);
		goto err_hw_rx_queue_setup;
		}

	/* arrange queue in intc array and rx_queue array in private
	 * device structure.
	 */
	priv->rx_queue[idx] = hw_rxq;
	priv->in_tcs[hw_rxq->tc].queues[hw_rxq->queue_idx] = hw_rxq;
	priv->in_tcs[hw_rxq->tc].bp_qs[hw_rxq->queue_idx] =
							&priv->bp_queue[idx];
	priv->in_tcs[hw_rxq->tc].bp_qs[hw_rxq->queue_idx]->tc = hw_rxq->tc;
	priv->in_tcs[hw_rxq->tc].bp_qs[hw_rxq->queue_idx]->queue_idx = idx;

	/* Initiate and fill the buffer pool queue */
	priv->bp_queue[idx].priv = priv;
	priv->bp_queue[idx].queue_type = ARMADA_EP_BPOOL_QUEUE;

	desc = RTE_MIN(desc, priv->bp_max_size);
	priv->bp_init_size = desc;
	priv->bp_queue[idx].count = desc;
	priv->bp_queue[idx].bp_frag_size = priv->buff_size;

	desc_size = sizeof(struct armada_ep_bpool_desc);
	ret = armada_ep_alloc_queue_resources(&priv->bp_queue[idx], desc_size,
					      priv->rx_queue_size, 0);
	if (ret) {
...
err_hw_rx_queue_setup:
	if (hw_rxq) {
		armada_ep_free_queue_resources(hw_rxq);
		armada_ep_free_queue_resources(&priv->bp_queue[idx]);
		rte_free(hw_rxq);
	}
	return ret;
}
			 ...
			 unsigned int socket, const struct rte_eth_txconf *conf)
{
	...
	hw_txq->last_tx_cons_val = 0;

	desc_size = sizeof(struct armada_ep_desc);
	ret = armada_ep_alloc_queue_resources(hw_txq, desc_size,
					      priv->tx_queue_size, 0);
	if (ret) {
		ARMADA_EP_LOG(ERR, "Failed to allocate tx ring %d.\n", idx);
		goto err_hw_tx_queue_setup;
	}

	/* arrange queue in outtc array and tx_queue array in private
	 * device structure.
	 */
	priv->tx_queue[idx] = hw_txq;
	priv->out_tcs[hw_txq->tc].queues[hw_txq->queue_idx] = hw_txq;
...
err_hw_tx_queue_setup:
	armada_ep_free_queue_resources(hw_txq);
	rte_free(hw_txq);
	return ret;
}
...
/**
 * armada_ep_init_qeueu_indeces - Initialize Q pointers page.
 *
 * @param priv
 *   Pointer to the private device structure.
 *
 */
static inline int
armada_ep_init_qeueu_indeces(struct armada_ep_priv *priv)
{
	uint32_t array_size;
 ...
 */
static int armada_ep_setup_mgmt_qs(struct armada_ep_priv *priv)
{
	...
	ret = armada_ep_alloc_queue_resources(queue,
					      sizeof(struct armada_ep_cmd_desc),
					      ARMADA_EP_CMD_QUEUE_LEN, 1);
	if (ret) {
 ...
 */
static inline int
armada_ep_priv_init(struct armada_ep_priv *priv, int device_id, void *nic_va)
{
	int ret;
	...
	priv->dev_initialized = 0;
		...
		/* Request device to reset Host config and start new process */
		writel(ARMADA_EP_CFG_STATUS_HOST_RESET, &priv->nic_cfg->status);

		/* Wait until the device firmware sets up the BARs */
		do {
			//status = readl(&priv->nic_cfg->status);
			//if (status &
			if (readl(&priv->nic_cfg->status) &
	...
	/* set indeces */
	ret = armada_ep_init_qeueu_indeces(priv);
	if (ret)
		goto err_priv_init;

	// TODO: verify if the below mac update is neseccery
	/* Get the MAC address out of the NIC configuration space. */
	/* copy the eth-addr with 4B alignment in order to prevent issues with
	 * some PCIe busses
	 */

	//priv->mac = *priv->nic_cfg->mac_addr;
	//&priv->mac[4] =	*&priv->nic_cfg->mac_addr[4];

#if !ARMADA_EP_STANDALONE_VDEV_MODE
	ret = armada_ep_setup_mgmt_qs(priv);
	if (ret)
		goto err_mgmt_qs;
#endif
			 ...
			 void *nic_va)
{
	...
	ret = armada_ep_priv_init(priv, device_id, nic_va);
	if (ret) {
...
armada_ep_eth_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	return ret;
out_cleanup:
	ARMADA_EP_LOG(ERR, "Failed to init nix eth_dev ret=%d", ret);
