...
int
octeontx_pki_send_mtu(int port, pki_mtu_cfg_t *mtu_cfg)
{
	struct octeontx_mbox_hdr hdr;
	pki_mtu_cfg_t cfg = *mtu_cfg;
	int res;

	hdr.coproc = OCTEONTX_PKI_COPROC;
	hdr.msg = MBOX_PKI_PORT_MTU_CONFIG;
	hdr.vfid = port;

	res = octeontx_mbox_send(&hdr, &cfg, sizeof(pki_mtu_cfg_t), NULL, 0);
	if (res < 0)
		return -EACCES;
