...
octeontx_dev_mtu_set(struct rte_eth_dev *eth_dev, uint16_t mtu)
{
	...
	mtu_cfg.port_type = OCTTX_PORT_TYPE_NET;
	mtu_cfg.maxlen = frame_size;
	mtu_cfg.minlen = OCCTX_MIN_HW_FRS;

	rc = octeontx_pki_send_mtu(nic->port_id, &mtu_cfg);
	if (rc)
		return rc;
	...
	octeontx_log_info("Received pkt beyond range maxlen %d minlen %d will be dropped",
	       mtu_cfg.maxlen, mtu_cfg.minlen);
